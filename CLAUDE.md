...
**Same bad host on every re-provision**: Compare `machineId` across attempts. RunPod will happily place a replacement pod back on the machine that just failed. If the ID is unchanged and egress keeps breaking, terminate and re-provision until it changes, or file a support ticket citing the ID. The prebaked fast path needs no network, but `download_gbif_images.py` (GBIF API) and any R2 push do.
